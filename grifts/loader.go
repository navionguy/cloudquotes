...
func createConversation(cv conversationtype) error {
	...
	conv.Publish = (strings.Compare("True", cv.Conversation[0].Publish) == 0)
	tracemsg(fmt.Sprintf("creating conversation ID = %s", conv.ID), 4)
...
func createQuote(cv uuid.UUID, sequence int, qt utterancestype) error {
	...
	aQuote := &models.Quote{
		...
		Phrase:         qt.Quote,
		AuthorID:       authID,
		Publish:        strings.Compare("True", qt.Publish) == 0,
		ConversationID: cv,
		Annotation:     nil,
		AnnotationID:   nil,
	}
